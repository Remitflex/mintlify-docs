...
Failed swaps return `status: failed` with a sanitized `errorMessage` on the swap object and in transaction `receipt` — not raw on-chain simulation output. Contact support with the swap ID if a deposit was received but conversion did not complete.
