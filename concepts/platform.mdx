...
| Scope | Access |
|-------|--------|
| `fx:read` | Fiat sell quotes (`/v1/fiat/rates/...`) |
| `transfers:read` | Read customers, transactions |
| `transfers:write` | Create customers |
...
- **Outbound webhooks** — poll order endpoints or the transactions ledger for now
- **Self-serve API key creation via API** — keys are created in the dashboard
- **Custodial wallets** — scopes exist; wallet APIs are not live

The API surface will expand without breaking existing v1 contracts where possible.
