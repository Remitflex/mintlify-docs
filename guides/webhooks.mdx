...
<Warning>
  **Not yet available.** Outbound merchant webhooks are not implemented in the RemitFlex API today. Poll `GET /swaps/{id}` or `GET /payment-routes/{id}/deposits` for status updates. This page describes the planned webhook product.
</Warning>

Webhooks are HTTP callbacks that RemitFlex sends to your server when something happens.
