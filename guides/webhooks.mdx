...
Manage endpoints in the [merchant app](https://app.remitflex.io/webhooks) under **Webhooks**, or via the API with JWT or an API key that includes `webhooks:read` / `webhooks:write`.
...
| Test | `webhook.test` (from `POST /v1/webhooks/:id/test` — delivered to that endpoint even if it is not on the allowlist) |

Every subsequent status change emits `*.updated` (including cancel/expire), not only terminal states. `cngn.customer.enabled` / `.failed` fire once when KYC finishes.
...
- Inbound provider callbacks (PayCrest, StRails, Relay) use separate routes under `/v1/webhooks/{provider}` and are not merchant outbound endpoints. Payment route, collection, and swap status updates also arrive via Relay push (`POST /v1/webhooks/relay`) with poll jobs as fallback.
