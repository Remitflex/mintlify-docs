...
| cNGN | `cngn.convert.*`, `cngn.withdrawal.*`, `cngn.payout.*`, `cngn.payin.*`, `cngn.deposit.*` (`.created` / `.updated`) |
...
- Inbound provider callbacks (separate routes) are not merchant outbound endpoints.
