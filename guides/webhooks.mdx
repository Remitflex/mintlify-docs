...
- Inbound provider callbacks (PayCrest, StRails, Relay) use separate routes under `/v1/webhooks/{provider}` and are not merchant outbound endpoints. Payment route and collection status updates also arrive via Relay push (`POST /v1/webhooks/relay`) with poll jobs as fallback.
