...
<Warning>
  **Not yet available.** Off-ramp to bank accounts is not implemented in the RemitFlex API today. Use [payment routes](/guides/send-payment) for cross-chain collection or [swaps](/guides/currency-conversion) for Solana token conversion. This page describes the planned product direction.
</Warning>

Off-ramping is the process of converting stablecoins into a local fiat currency and depositing the proceeds into a real bank account.
