...
|----------|---------|-------|
| `GET /v1/fiat/currencies` | Supported local currencies | `api:read` |
| `GET /v1/fiat/institutions/:code` | Banks for a currency | `api:read` |
| `POST /v1/fiat/verify-account` | Confirm account name before payout | `api:write` |
| `GET /v1/fiat/rates/:token/:amount/:currency` | Sell quote (stablecoin → fiat) | `api:read` |
...
NGN is live today. Additional currencies appear in `/fiat/currencies` as they become available.
...
List orders: `GET /v1/offramps?customerId=...`
...
List orders: `GET /v1/onramps?customerId=...`
...
Filter by `customerId` on `GET /v1/transactions`, `GET /v1/offramps`, and `GET /v1/onramps` to build per-user payout or funding history.

For status updates, see [Webhooks](/guides/webhooks).
