...
Reload the customer with `GET /v1/customers/{id}`, or subscribe to webhooks.
...
| Need | Use |
|------|-----|
| Live wallet balances | `GET /v1/cngn/customers/{id}/wallets` (or dashboard **Refresh**). Primary balances are live; the wallet list is a retained cache — RemitFlex does not drop addresses just because StRails stopped listing them. |
