...
Enablement is **asynchronous**. When KYC finishes, RemitFlex emits `cngn.customer.enabled` (or `cngn.customer.failed`). The customer object includes `cngn: { enabled, status, evmWallet, virtualAccount, syncedAt }` — `null` when cNGN has never been started.
