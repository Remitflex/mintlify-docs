...
Refresh the local cache (VA, wallet list, deposit/payout ledger) with `POST /v1/customers/{id}/strails/sync`. Dashboard history and wallet metadata read from that cache. `GET /v1/customers/{id}/strails/wallets` still fetches **live balances** only.
