...
| `GET` | `/collections/{collectionId}` | Get link; checkout parents include `sessions[]` |
| `PATCH` | `/collections/{collectionId}` | Update metadata, embed theme, or cancel |
| `POST` | `/collections/{collectionId}/sync` | Pull latest deposit status from chain |
| `POST` | `/collections/{collectionId}/logo` | Upload logo (multipart, max 2 MB) |
...
Share `payUrl`. The payer quotes and pays on that same id. Poll `GET /collections/{collectionId}` or `GET /pay/{collectionId}` for status (for invoices, both use the same `{collectionId}`).
...
Use **`PATCH /collections/{collectionId}`** on the **parent** link (invoice or checkout). Sessions cannot be PATCHed.

```bash
# Edit labels
curl -X PATCH https://api.remitflex.io/v1/collections/{collectionId} \
  -H "Authorization: Bearer $REMITFLEX_API_KEY" \
  -H "Content-Type: application/json" \
...
**Logo** — `POST /collections/{collectionId}/logo` (multipart). **Sync** — `POST /collections/{collectionId}/sync`. Legacy aliases `POST .../cancel` and `PATCH .../embed-config` still work.

## Payer endpoints

No auth.

On pay routes, `{collectionId}` is the payment link UUID — the invoice id, checkout **parent** id (for quote), or checkout **session** id (after quote).

| Method | Path | Purpose |
|--------|------|---------|
| `GET` | `/pay/{collectionId}` | Pay page data (destination, options, status) |
| `POST` | `/pay/{collectionId}/quote` | Choose origin + return address → deposit address |
| `GET` | `/pay/{collectionId}/logo` | Public logo image |
| `GET` | `/pay/{collectionId}/embed-config` | Theme for embed iframe |

```bash
curl -X POST https://api.remitflex.io/v1/pay/{collectionId}/quote \
  -H "Content-Type: application/json" \
  -d '{
    "originChainKey": "tron",
    "originCurrency": "USDT",
...
Each field has `required: true` or `false`. On quote (`POST /pay/{collectionId}/quote`), send collected values in `payerDetails`:
...
Collected `payerDetails` appear on `GET /collections/{collectionId}` and checkout session lists.
...
On `completed`, `refunded`, and `failed`, Remitflex emails a receipt to the creating dashboard user. When **customer receipts** are enabled for the organisation (dashboard **Settings**, or `PATCH /v1/org/settings`), the payer (if an email was collected) and linked customer are emailed too.
...
1. **Theme** — `PATCH /collections/{collectionId}` (`embedConfig`) or dashboard **Payment Links → Embed**
2. **Loader** — `embed.js` opens a modal or inline iframe at `/embed/{linkId}`
3. **Pay UI** — theme from `GET /pay/{collectionId}/embed-config` (checkout sessions inherit parent theme)
